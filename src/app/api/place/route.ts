...
export async function POST(req: Request) {
  ...
  try {
    for (const place of places) {
      const url = `https://maps.googleapis.com/maps/api/place/findplacefromtext/json?fields=formatted_address,geometry,photos&input=input=${encodeURIComponent(
        `${place.name} ${to}`
    ...
    const hotelResponse = await fetch(
      "https://places.googleapis.com/v1/places:searchNearby",
      {
        ...
        headers: {
          ...
          "X-Goog-FieldMask":
            "places.displayName,places.location,places.formattedAddress,places.photos,places.rating",
        },
        body: JSON.stringify({
